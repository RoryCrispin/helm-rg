...
(defconst helm-rg--ripgrep-argv-format-alist
  `((helm-rg-ripgrep-executable :face helm-rg-base-rg-cmd-face)
    ...
    ((unless (helm-rg--empty-glob-p helm-rg--glob-string)
     ...
     :face helm-rg-active-arg-face)
    ((unless (helm-rg--empty-glob-p helm-rg--exclusion-string)
       (concat "-g" "!{" helm-rg--exclusion-string "}"))
     :face helm-rg-active-arg-face)
...
(defvar helm-rg--exclusion-string nil
...
(defun helm-rg--set-exclusion-glob  (excl-str)
  "Set the inverse glob string used to invoke ripgre, then search again.
  The currently focused file will be used as the default input to the exclusion."
  (interactive (list nil))
  (helm-rg--set-setting-and-restart helm-rg--exclusion-string
    (or excl-str
        (with-helm-buffer
          (cl-destructuring-bind (&key file) (helm-rg--current-jump-location)
            (read-string
             "exclude globs: "
             (concat helm-rg--exclusion-string (if (equal nil helm-rg--exclusion-string) "" ",") file)))))))
...
(defconst helm-rg-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "M-e") #'helm-rg--set-exclusion-glob)
