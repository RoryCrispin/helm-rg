...
;; - Use `M-e' to exclude a glob pattern from the search results. The input will
;; be initially populated with the currently selected result, and any previously
;; excluded glob patterns for this search session. This is often useful for
;; quickly hiding directories while searching for a specific match.
...
(defconst helm-rg--ripgrep-argv-format-alist
  `((helm-rg-ripgrep-executable :face helm-rg-base-rg-cmd-face)
    ...
    ((unless (helm-rg--empty-glob-p helm-rg--glob-string)
     ...
     :face helm-rg-active-arg-face)
    ((unless (helm-rg--empty-glob-p helm-rg--exclusion-glob-string)
       (concat "-g" "!{" helm-rg--exclusion-glob-string "}"))
     :face helm-rg-active-arg-face)
...
(defvar helm-rg--exclusion-glob-string nil
  "Inverse glob string used for the current `helm-rg' session.")
...
(defun helm-rg--unwind-cleanup ()
  ...
  (setq helm-rg--glob-string nil
        helm-rg--exclusion-glob-string nil
...
(defun helm-rg--set-exclusion-glob-string  (excl-str)
  "Set the inverse glob string used to invoke ripgrep, then search again.
  The currently focused file will be used as the default input to the exclusion.
  If there is already an exclusion string set, then the current file will be appended to it."
  (interactive (list nil))
  (helm-rg--set-setting-and-restart helm-rg--exclusion-glob-string
    (or excl-str
        (with-helm-buffer
          (cl-destructuring-bind (&key file) (helm-rg--current-jump-location)
            (read-string
             "exclude globs: "
             (concat helm-rg--exclusion-glob-string (if (equal nil helm-rg--exclusion-glob) "" ",") file)))))))
...
(defconst helm-rg-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "M-e") #'helm-rg--set-exclusion-glob-string)
